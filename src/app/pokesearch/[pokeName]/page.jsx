...
function PokeSearch() {
    ...
    const fetchPokeData = async () => {
        try {
            ...
            setError(null);
            setImageError(false);
            const response = await fetch(`https://pokeapi.co/api/v2/pokemon/${params.pokeName.toLowerCase()}`);
            if (!response.ok) {
            ...
            <div className='bg-white/80 backdrop-blur-sm shadow-xl p-8 md:p-12 rounded-2xl transform hover:scale-[1.02] transition-all duration-300 max-w-2xl w-full border border-white/20'>
                {loading ? (
                    ...
                    <div className="text-center py-8">
                        ...
                        />
                        <h3 className="text-2xl font-bold text-red-400 mb-4 font-semibold">Pokémon Name Not Found</h3>
                        <p className="text-gray-600 font-semibold">Please try searching for a different Pokémon name.</p>
                    </div>
                ) : pokeData && (
                    <div className="space-y-6">
                        <h3 className='text-4xl md:text-5xl font-bold text-gray-700 capitalize'>{pokeData.name}</h3>
                        <div className="relative w-64 h-64 mx-auto">
                            {!imageError ? (
                                <Image 
                                    ...
                                    alt={pokeData.name || "Pokemon image"}  
                                    width={300} 
                                    height={300} 
                                    loading="eager"
                        ...
                        <div className='bg-gradient-to-r from-purple-50 to-blue-50 p-4 rounded-xl border border-purple-100 space-y-4'>
                            <div className='flex flex-col justify-center items-center space-x-2 flex-wrap gap-2'>
                                <p className='text-lg font-semibold text-gray-700'>Height: {pokeData.height * 10} cm</p> 
                                <p className='text-lg font-semibold text-gray-700'>Weight: {pokeData.weight / 10} kg</p>
                            </div>
